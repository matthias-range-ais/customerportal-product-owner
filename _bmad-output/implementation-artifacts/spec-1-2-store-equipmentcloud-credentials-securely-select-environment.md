...
- **User feedback after manual testing (post-review):** the Windows Credential Manager entries show the account name as literally `test`/`prod`, not the EquipmentCloud username actually entered. This is expected given the adapter design (Code Map: `account` is a fixed lookup key so the app can find the entry without already knowing the username; the real username+password are inside the encrypted password blob, which the Windows Credential Manager UI doesn't decode). Asked the user to choose between keeping this behavior or displaying the real username (which would require a small non-secret local file mapping environment → username, so the app knows which account name to query). **Decision: keep current behavior, no change.** The real username is still stored and used correctly for authentication (from Story 1.3 onward) — only the Windows GUI's display of it is affected.
- **User feedback after manual testing:** current UI has no visual design ("super hässlich" / "wanted a modern design"). Also raised that the settings screen occupying the entire app (no separate dialog/navigation) is confusing; accepted as fine for now, explicitly deferred to when a second screen is needed (at latest Story 1.4) rather than building navigation infrastructure speculatively now — see `deferred-work.md`.
- **Post-feedback styling pass:** rewrote `frontend/src/index.css` (new color tokens — blue accent, success/danger/neutral colors, card shadow — replacing the leftover Vite-template palette; light and dark mode both defined) and restructured `App.tsx`'s markup into a `.page` → `.card`-per-environment layout with status `.badge`s (configured/not-configured/active), labeled `.field`s, and primary/secondary buttons. Verified visually with Playwright screenshots in both light and dark mode (`prefers-color-scheme`) against the real running server — renders as clean, modern cards with clear status indicators, not the previous unstyled HTML. Fixed 3 frontend tests that broke because the active-environment text is now split across a `<p>`/`<strong>` (added `data-testid="active-environment"` and switched those assertions to `toHaveTextContent`, which handles nested markup correctly instead of `getByText`'s exact-string matching).
